...
def create_patients_table(cur, conn):
    ...
    cur.execute("CREATE TABLE Patients (pet_id INTEGER PRIMARY KEY, name TEXT,\
         species_id INTEGER, age INTEGER, cuteness INTEGER, agressiveness INTEGER)")
...
def add_pets_from_json(filename, cur, conn):
    ...
    speciesDic = {}
    for tup in species:
        speciesDic[tup[1]] = tup[0]
    for i in range(len(json_data)):
        cur.execute("INSERT INTO Patients (id,title) VALUES (?,?)",(i,json_data[i],json_data[i]["name"],
        speciesDic[json_data[i]["species"]],))
    pass
